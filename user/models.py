...
class CustomUserManager(UserManager):
    ...
    def _create_user(self, username, password, **extra_fields):
        if not username:
            ...
            raise ValueError("Please create another username.")
...
class Department(models.Model):
    ...
    PESO      = "Public Employment Safety"
    MTO      = "Municipal Treasury"
    MO      = "Mayor's Office"
    ...
    choices = [
        ...
        (MTO, MTO),
        (MO, MO),
        # add other depts if needed
        ]

    name = models.CharField(blank=True, null=False, max_length=80, choices=choices, default=select, verbose_name="Department: ")
    dept_head = models.ForeignKey(DepartmentHead, on_delete=models.SET_NULL, null=True, blank=False)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(max_length=100, unique=True) # change max_length if needed, add min_length/value
    email = models.EmailField() # add unique & null field options in production!
    ...
    def dp_directory_path(instance, filename):
        ...
        return 'users/{}/DP/{}'.format(instance.username, filename)
    ...
    def get_absolute_url(self):
        return reverse('user', kwargs={'pk': self.username})
